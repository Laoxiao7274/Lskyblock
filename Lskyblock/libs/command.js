...
function init_iland() {
    ...
    is.mandatory("action", ParamType.Enum, "op", 1);
    ...
    is.setCallback((cmd, ori, output, res) => {
        ...
        switch (res.action) {
            ...
            case "invite":
                if (File.exists("./plugins/Lskyblock/dataBase/island/" + ori.player.name + "/")) {
                    const conf = new JsonConfigFile("./plugins/Lskyblock/dataBase/island"+ori.player.name+"/land.json");
            ...
            case "accept":
                if (ivt_pl.name == ori.player.name) {
                    if (ivt) {
                        Invite.invite(master_pl.name, ivt_pl,master_pl);
                        ivt = false;
                        ivt_pl = {};
                    }
                    else {
            ...
            case "sethome":
                if (is_land(ori.player)) {
                    ...
                    Home.sethome(ori.player.name,ori.player);
                }
                break;
            case "help":
                if (ori.player.isOP()) {
                    ...
                    ori.player.tell("§a--------------------------");
                    ori.player.tell("§a/is help                         空岛帮助");
                    ori.player.tell("§a/is op                           总空岛设置");
                    ...
                    ori.player.tell("§a/is range add/remove 玩家名 距离   更改岛屿范围");
                    ori.player.tell("§a--------------------------");
                }
                else {
                    ori.player.tell("§a--------------------------");
            ...
            case "share":
                if (res.add == "add") {
                    if (ori.player.name != res.name[0].name) {
                        Share.share(ori.player.name, res.name[0],ori.player);
                    }
                    else {
                    ...
                    if (ori.player.name != res.name[0].name) {
                        Share.remove(ori.player.name, res.name[0],ori.player);
                    }
                    else {
                        ori.player.tell("§4请不要输入自己的名字");
                    }
            ...
            case "range":
                if (ori.player.isOP()) {
                    if (res.add == "add") {
                        if (File.exists("./plugins/Lskyblock/dataBase/island/" + res.string + "/")) {
                            const pl_land_conf = new JsonConfigFile("./plugins/Lskyblock/dataBase/island/" + res.string + "/land.json");
                            let change_range = pl_land_conf.get("island_range");
                            change_range += res.number;
                            pl_land_conf.set("island_range", change_range);
                            ori.player.tell("§a已成功为你添加范围");
                        }
                    }
                    else if (res.add == "remove") {
                        const pl_land_conf = new JsonConfigFile("./plugins/Lskyblock/dataBase/island/" + res.string + "/land.json");
                        ...
                        change_range -= res.number;
                        pl_land_conf.set("island_range", change_range);
                        ori.player.tell("§a已成功为你减少范围");
                    }
                }
                else {
                    ori.player.tell("§4你没有权限这么做");
                }
                break;
            case "op":
                if (ori.player.isOP()) {
                    Form.op_form(ori.player);
                }
                else {
                    ori.player.tell("§4你没有权限这么做");
                }
